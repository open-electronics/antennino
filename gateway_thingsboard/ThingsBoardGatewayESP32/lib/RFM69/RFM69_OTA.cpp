...
uint8_t programmer_is_busy = 0;
uint8_t program_process_is_complete = 0;


uint8_t ProgrammerIsBusy(){
  return programmer_is_busy;
}


uint8_t ProgramProcessIsComplete(){
  return program_process_is_complete;
}
...
void CheckForWirelessHEX(RFM69& radio, SPIFlash& flash, uint8_t DEBUG, uint8_t LEDpin){
  ...
  if (radio.DATALEN >= 4 && radio.DATA[0]=='F' && radio.DATA[1]=='L' && radio.DATA[2]=='X' && radio.DATA[3]=='?'){
    ...
    {
		if (DEBUG) {
			Serial.print(F("FLASH IMG TRANSMISSION SUCCESS!\n"));
		}
       ...
       resetUsingWatchdog(DEBUG);
		...
		if (DEBUG) {
			Serial.print("Timeout/Error, erasing written data ... ");
		}
...
void HandleHandshakeACK(RFM69& radio, SPIFlash& flash, uint8_t flashCheck) {
  ...
  if (flashCheck){
    ...
    if (!flash.initialize()){
	  ...
	  //NO FLASH CHIP FOUND, ABORTING
      radio.sendACK("FLX?NOK:NOFLASH",15); 
...
uint8_t HandleWirelessHEXDataWrapper(RFM69& radio, uint8_t remoteID, SPIFlash& flash, uint8_t DEBUG, uint8_t LEDpin) {
  ...
  if (DEBUG) {
	  Serial.println(F("FLX?OK (ACK sent)")); 
	  Serial.print(F("Shifting channel to "));
	  Serial.println(radio.getFrequency() + SHIFTCHANNEL);
  }

  //shift center freq by SHIFTCHANNEL amount
  radio.setFrequency(radio.getFrequency() + SHIFTCHANNEL); 

  uint8_t result = HandleWirelessHEXData(radio, remoteID, flash, DEBUG, LEDpin);

  if (DEBUG) { 
	  Serial.print(F("UNShifting channel to "));
	  Serial.println(radio.getFrequency() - SHIFTCHANNEL);
  }
...
uint8_t HandleWirelessHEXData(RFM69& radio, uint8_t remoteID, SPIFlash& flash, uint8_t DEBUG, uint8_t LEDpin) {
  ...
  uint16_t tmp,seq=0;
  char buffer[16];
  uint16_t timeout = 7000; //3s for flash data
  ...
  while(1){
    ...
    if (radio.receiveDone() && radio.SENDERID == remoteID){
      ...
      if (dataLen >= 4 && radio.DATA[0]=='F' && radio.DATA[1]=='L' && radio.DATA[2]=='X'){
		  ...
		  //FLX:_:_
		  if (radio.DATA[3]==':' && dataLen >= 7) {
          uint8_t index=3;
          tmp = 0;
          
          //read packet SEQ
		      //up to 4 characters for seq number
        ...
        if (radio.DATA[3]=='?'){
		       ...
		       //ACK for handshake was lost, resend
          if (dataLen==4) {
            HandleHandshakeACK(radio, flash);
      			if (DEBUG) {
      				Serial.println(F("FLX?OK resend"));
      			}			
          }

		    //Expected EOF
        if (dataLen==7 && radio.DATA[4]=='E' && radio.DATA[5]=='O' && radio.DATA[6]=='F') {
          
        #if defined (MOTEINO_ZERO)
            if ((bytesFlashed-10)>253952) { //max 65536 - 10 bytes (signature)
      				if (DEBUG) {
      					Serial.println(F("IMG > 64k, too big"));
      				} 
              radio.sendACK("FLX?NOK:HEX>64k",15);
              return false; //just return, let MAIN timeout
            }
        #elif defined(__AVR_ATmega1284P__)
            //max 65536 - 10 bytes (signature)
            if ((bytesFlashed-10)>65526) { 
      			  if (DEBUG) {
      					Serial.println(F("IMG > 64k, too big"));
      			  }
              radio.sendACK("FLX?NOK:HEX>64k",15); 
        ...
        if ((bytesFlashed-10)>31744) {
      				if (DEBUG) {
      					Serial.println(F("IMG > 31k, too big"));
      				}
  ...
  if (inputLen == 4 && input[0]=='F' && input[1]=='L' && input[2]=='X' && input[3]=='?') {
    ...
    if (HandleSerialHandshake(radio, targetID, false, TIMEOUT, ACKTIMEOUT, DEBUG)){
      ...
      if (radio.DATALEN >= 7 && radio.DATA[4] == 'N'){
		//signal serial handshake fail/error and return
	...
	#endif
		  {
			//signal EOF serial handshake back to host script
			Serial.println(F("FLX?OK")); 
			if (DEBUG) {
...
//===================================================================================================================
// CheckForFlashHEX() - returns TRUE if a HEX file transmission was detected and it was actually transmitted successfully
// this is called at the OTA programmer side
//TODO:MIRKO
//===================================================================================================================
uint8_t CheckForFlashHEX(SPIFlash flash, RFM69& radio, uint8_t targetID, uint16_t TIMEOUT, uint16_t ACKTIMEOUT, uint8_t DEBUG) {

programmer_is_busy =1;
program_process_is_complete = 0;

if (HandleFlashHandshake(radio, targetID, false, TIMEOUT, ACKTIMEOUT, DEBUG)) {

	if (radio.DATALEN >= 7 && radio.DATA[4] == 'N') {
        programmer_is_busy =0; 
		return false;
	}

	//signal serial handshake back to host script
	//Serial.println(F("\nFLX?OK")); 

#ifdef SHIFTCHANNEL
	if (HandleFlashHEXDataWrapper(flash, radio, targetID, TIMEOUT, ACKTIMEOUT, DEBUG))
#else
	if (HandleFlashHEXData(flash, radio, targetID, TIMEOUT, ACKTIMEOUT, DEBUG))
#endif
	{
        //signal EOF serial handshake back to host script
		//Serial.println(F("FLX?OK")); 
...
uint8_t HandleSerialHandshake(RFM69& radio, uint8_t targetID, uint8_t isEOF, uint16_t TIMEOUT, uint16_t ACKTIMEOUT, uint8_t DEBUG){
  ...
  if (DEBUG) {
	  Serial.println(F("Handshake fail"));
  }
...
//===================================================================================================================
// HandleFlashHandshake() - handles the handshake with the serial port
//TODO:MIRKO
//===================================================================================================================
uint8_t HandleFlashHandshake(RFM69& radio, uint8_t targetID, uint8_t isEOF, uint16_t TIMEOUT, uint16_t ACKTIMEOUT, uint8_t DEBUG){
	long now = millis();
...
//===================================================================================================================
// HandleFlashHEXDataWrapper() - wrapper for HandleFlashHEXData(), also shifts the channel if SHIFTCHANNEL is defined
//TODO:MIRKO
//===================================================================================================================
#ifdef SHIFTCHANNEL
uint8_t HandleFlashHEXDataWrapper(SPIFlash flash, RFM69& radio, uint8_t targetID, uint16_t TIMEOUT, uint16_t ACKTIMEOUT, uint8_t DEBUG) {
	radio.setFrequency(radio.getFrequency() + SHIFTCHANNEL); //shift center freq by SHIFTCHANNEL amount
	uint8_t result = HandleFlashHEXData(flash,  radio, targetID, TIMEOUT, ACKTIMEOUT, DEBUG);
	radio.setFrequency(radio.getFrequency() - SHIFTCHANNEL); //shift center freq by SHIFTCHANNEL amount
	return result;
}
#endif
    ...
    if (inputLen >= 6) { 
      ...
      if (input[0]=='F' && input[1]=='L' && input[2]=='X'){
        ...
        if (input[3]==':'){
          ...
          uint8_t hexDataLen = validateHEXData(input+index, inputLen-index, DEBUG);

          if (hexDataLen>0 && hexDataLen<253){
            
            //only read data when packet number is the next expected SEQ number
            if (tmp==seq) {  

			  //extract HEX data from input to BYTE data into sendBuf (go from 2 HEX bytes to 1 byte), +8 jumps over the header to the HEX raw data	 
...
//TODO:MIRKO
//===================================================================================================================
uint8_t HandleFlashHEXData(SPIFlash flash, RFM69& radio, uint8_t targetID, uint16_t TIMEOUT, uint16_t ACKTIMEOUT, uint8_t DEBUG) {
	uint16_t seq;
	uint8_t remoteID = radio.SENDERID; //save the remoteID as soon as possible
	uint8_t sendBuf[57];
	long inputLen = 0;
  ...
  do {
        value = flash.readByte(i);
		if (value != 10) {
      
        //Accumo i dati nell'Array
		input[counter++] = value;
		inputLen++;  
     
		} else {
    			
         /// <-- Corpo su cui ciclare-->
              if (data_row == 0) {
                seq = 0;
              }

              //a FLASH record should not be more than 64 bytes: :10042000FF4FA591B4912FB7F894662321F48C91D6 
              uint8_t hexDataLen = validateHEXData(input + 1, inputLen - 2, DEBUG);
              ...
              if (hexDataLen>0 && hexDataLen<253) { 
                ...
                if (data_row == seq) {
					  ...
					  //extract HEX data from input to BYTE data into sendBuf (go from 2 HEX bytes to 1 byte), +8 jumps over the header to the HEX raw data
					  uint8_t sendBufLen = prepareSendBuffer(input + 1 + 8 , sendBuf, hexDataLen, seq);
					  
					  //Serial.print(F("PREP "));Serial.print(sendBufLen); Serial.print(F(" > ")); 
					  
					  //PrintHex83(sendBuf, sendBufLen);
                ...
                //Serial.print(F("FLX:INV:"));
                return HandleFlashHandshake(radio, targetID, true, TIMEOUT, ACKTIMEOUT, DEBUG);
              }
...
//
//   :CCAAAATTXXXXX......XXSS    es    :100000000C94BF000C9446110C946D110C94E700F5   :10 0000 00 0C94BF000C9446110C946D110C94E700 F5
//   CC = (Character Count) in Byte
//   AAAA =   starting address
//   TT= 01  Last Row  = 00 There are more  row
//   XXXXX......XX  = Payload Data 
//   SS = Chechsum
//===================================================================================================================
uint8_t validateHEXData(void* data, uint8_t length, uint8_t DEBUG){
  ...
  if (DEBUG) {
	  Serial.print(F("VAL>")); Serial.println((char*)data);
  }

  if (length < 12 || length % 2 != 0) {
	  return 0; //shortest possible intel data HEX record is 12 bytes
  }
  ...
  for (uint8_t i=0; i<length;i++){
    ...
    //0-9,A-F  Verifico che siano solo caratteri Esadecimali
	if (!((input[i] >= 48 && input[i] <= 57) || (input[i] >= 65 && input[i] <= 70))) {
		  return 255;
	}
      
	if (i % 2 && i < length - 2) { 
		checksum += BYTEfromHEX(input[i - 1], input[i]);
	}
  ...
  //length of actual HEX flash data (usually 16bytes)
  uint8_t dataLength = BYTEfromHEX(input[0], input[1]); 

  //calculate record length
  //add headers and checksum bytes (a total of 10 combined)
  if (length != dataLength * 2 + 10) {
	  return 253;
  }
...
uint8_t sendHEXPacket(RFM69& radio, uint8_t targetID, uint8_t* sendBuf, uint8_t hexDataLen, uint16_t seq, uint16_t TIMEOUT, uint16_t ACKTIMEOUT, uint8_t DEBUG){
  ...
  while(1) {
    ...
    if (DEBUG) { 
		 Serial.print(F("RFTX > ")); PrintHex83(sendBuf, hexDataLen); 
    }

    if (radio.sendWithRetry(targetID, sendBuf, hexDataLen, 2, ACKTIMEOUT)){
      uint8_t ackLen = radio.DATALEN;   
      if (DEBUG) { 
        Serial.print(F("RFACK > ")); 
        Serial.print(ackLen); Serial.print(F(" > ")); 
        ((uint8_t*)radio.DATA, ackLen); 
      }
      if (ackLen >= 8 && radio.DATA[0]=='F' && radio.DATA[1]=='L' && radio.DATA[2]=='X' && radio.DATA[3]==':' && radio.DATA[ackLen-3]==':' && radio.DATA[ackLen-2]=='O' && radio.DATA[ackLen-1]=='K'){
        uint16_t tmp=0;
